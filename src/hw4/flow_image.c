...
image make_integral_image(image im) {
    ...
    // TD: fill in the integral image
    for (int c = 0; c < im.c; c++) {
        for (int y = 0; y < im.h; y++) {
            for (int x = 0; x < im.w; x++) {
                float left = x > 0 ? get_pixel(integ, x - 1, y, c) : 0.0;
                float up = y > 0 ? get_pixel(integ, x, y - 1, c) : 0.0;
                float up_left = (x > 0 && y > 0) ? get_pixel(integ, x - 1, y - 1, c) : 0.0;
                float val = get_pixel(im, x, y, c) + left + up - up_left;
...
image box_filter_image(image im, int s) {
    ...
    // TD: fill in S using the integral image.
    for (int c = 0; c < im.c; c++) {
        for (int y = 0; y < im.h; y++) {
            int up = y - s / 2;
            if (up < 0) {
            ...
            for (int x = 0; x < im.w; x++) {
                ...
                float a = get_pixel(integ, right, down, c);
                float b = get_pixel(integ, left, down, c);
                float d = get_pixel(integ, right, up, c);
                float e = get_pixel(integ, left, up, c);
                float val = a - b - d + e;
...
image time_structure_matrix(image im, image prev, int s) {
    ...
    // TD: calculate gradients, structure components, and smooth them
    ...
    for (int y = 0; y < im.h; y++) {
        for (int x = 0; x < im.w; x++) {
            ...
            float it = get_pixel(gt, x, y, 0);
            set_pixel(S, x, y, 0, powf(ix, 2));
            set_pixel(S, x, y, 1, powf(iy, 2));
...
image velocity_image(image S, int stride) {
    ...
    for(j = (stride-1)/2; j < S.h; j += stride){
        for(i = (stride-1)/2; i < S.w; i += stride){
            ...
            // TD: calculate vx and vy using the flow equation
            float vx = 0;
            float vy = 0;
